...
'''
    Top-level module which uses a CRF to classify. Handcrafted feature vectors for each sentence are fed to a linear layer to obtain emission scores for each class. Emission scores are fed to the CRF to generate optimal tag sequence.
'''
class CRF_Classifier(nn.Module):
    def __init__(self, n_tags, sent_emb_dim, sos_tag_idx, eos_tag_idx, pad_tag_idx = None, device = 'cuda'):
        super().__init__()
        
        self.device = device
        self.pad_tag_idx = pad_tag_idx
        
        self.feat2tag = nn.Linear(sent_emb_dim, n_tags)
        self.crf = CRF(n_tags, sos_tag_idx, eos_tag_idx, pad_tag_idx).to(self.device)
        
    def forward(self, x, y):
        ## x: list[batch_size, sents_per_example, feature_vector_size]
        ## y: list[batch_size, sents_per_example]
        
        # create tensors and pad
        ...
        tensor_y = nn.utils.rnn.pad_sequence(tensor_y, batch_first = True).to(self.device)
        
        mask = (tensor_y != self.pad_tag_idx).float().to(self.device)
        
        emissions = self.feat2tag(tensor_x)
        score, path = self.crf.decode(emissions, mask = mask)
        nll = self.crf(emissions, tensor_y, mask = mask)
        return nll, path
...
class Hier_LSTM_CRF_Classifier(nn.Module):
    def __init__(self, n_tags, sent_emb_dim, sos_tag_idx, eos_tag_idx, pad_tag_idx, vocab_size = 0, word_emb_dim = 0, pretrained = False, device = 'cuda'):
        super().__init__()
    ...
    def forward(self, x, y):
        ...
        if not self.pretrained: ## x: list[batch_size, sents_per_doc, words_per_sent]
            ...
            for doc in x:
                ...
                sents = nn.utils.rnn.pad_sequence(sents, batch_first = True, padding_value = word2idx['<pad>']).to(self.device)
        ...
        emissions = self.emitter(tensor_x)
